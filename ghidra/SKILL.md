...
3. Decompile the target and verify `complete: true`.
...
Large results are saved as artifacts. Read bounded slices from the complete `decompile.c`; treat block indexes, previews, and call lists only as navigation. Inspect every behavior-relevant branch before reconstructing omitted regions.
...
1. Retry an incomplete decompile once with a larger timeout.
2. Query `info`, then check for a thunk, offcut, wrong image base, stale project, encryption, or mismatched dump. Use only a short bounded disassembly for diagnosis.
...
- Do not bulk-enumerate functions, symbols, strings, callers, or xrefs unless the user explicitly requests a broad audit.
- Do not request callers for an unlabeled high-fanout runtime helper.
